...
public class DemoDataService {
    ...
    public void seedDatabase() {
        ...
        // images
        record Scenario(String title, String description, String img, String category, String tag, String fbCorrect,
                        String fbWrong) {
        }

        var scenarios = List.of(
                // CHEMISTRY - DANGEROUS
                new Scenario("Gefahrstoff ohne Kennzeichnung", "Auf einem Labortisch steht eine offene Flasche mit einer unbekannten Flüssigkeit. Niemand weiß, was sich darin befindet.", "demo/images/scenario-00.png", "CHEMISTRY", "DANGEROUS", "Richtig! Unbekannte Stoffe dürfen niemals ohne Kennzeichnung verwendet werden.", "Falsch. Unbekannte Chemikalien können gefährlich sein und dürfen nicht benutzt werden."),
                new Scenario("Experiment ohne Schutzbrille", "Ein Schüler führt ein Experiment durch, trägt aber keine Schutzbrille.", "demo/images/scenario-01.png", "CHEMISTRY", "DANGEROUS", "Genau! Im Chemieraum muss immer eine Schutzbrille getragen werden.", "Falsch. Chemikalien können in die Augen gelangen und schwere Verletzungen verursachen."),
                new Scenario("Wasser zur Säure", "Ein Schüler gießt Wasser direkt in konzentrierte Säure.", "demo/images/scenario-02.png", "CHEMISTRY", "DANGEROUS", "Richtig! Wasser darf nicht direkt zur Säure gegeben werden.", "Falsch. Dabei kann die Mischung spritzen und schwere Verätzungen verursachen."),
                new Scenario("Verschüttete Chemikalie ignorieren", "Auf dem Boden liegt eine verschüttete Chemikalie und niemand meldet den Vorfall.", "demo/images/scenario-03.png", "CHEMISTRY", "DANGEROUS", "Genau! Verschüttete Chemikalien müssen sofort gemeldet werden.", "Falsch. Verschüttete Stoffe können Verletzungen verursachen."),
                new Scenario("Brenner neben Alkohol", "Neben einem brennenden Bunsenbrenner steht eine offene Flasche mit Ethanol.", "demo/images/scenario-04.png", "CHEMISTRY", "DANGEROUS", "Richtig! Entzündliche Stoffe dürfen nie neben offenen Flammen stehen.", "Falsch. Es besteht Brand- und Explosionsgefahr."),

                // CHEMISTRY - SAFE
                new Scenario("Schutzbrille richtig aufgesetzt", "Vor dem Experiment setzen alle Schülerinnen und Schüler ihre Schutzbrillen auf.", "demo/images/scenario-05.png", "CHEMISTRY", "SAFE", "Genau! Schutzbrillen schützen die Augen vor Spritzern.", "Falsch. Das ist eine wichtige Sicherheitsmaßnahme."),
                new Scenario("Alle Behälter beschriftet", "Jede Flasche im Chemieraum besitzt ein gut lesbares Etikett.", "demo/images/scenario-06.png", "CHEMISTRY", "SAFE", "Richtig! Beschriftungen helfen dabei, Stoffe sicher zu verwenden.", "Falsch. Das ist ein sicheres Verhalten."),
                new Scenario("Schutzhandschuhe tragen", "Eine Schülerin zieht vor dem Experiment Schutzhandschuhe an.", "demo/images/scenario-07.png", "CHEMISTRY", "SAFE", "Genau! Handschuhe schützen die Haut vor Chemikalien.", "Falsch. Das erhöht die Sicherheit im Labor."),
                new Scenario("Sauberer Labortisch", "Nach dem Experiment räumt ein Schüler seinen Arbeitsplatz vollständig auf.", "demo/images/scenario-08.png", "CHEMISTRY", "SAFE", "Richtig! Ordnung reduziert Unfallrisiken.", "Falsch. Ein sauberer Arbeitsplatz ist sicher."),
                new Scenario("Unsicherheit melden", "Eine Schülerin fragt den Lehrer nach Hilfe, bevor sie einen unbekannten Stoff verwendet.", "demo/images/scenario-09.png", "CHEMISTRY", "SAFE", "Perfekt! Bei Unsicherheiten sollte immer nachgefragt werden.", "Falsch. Das ist die richtige Vorgehensweise."),
        ...
        for (int i = 0; i < scenarios.size(); i++) {
            ...
            // upload the bundled demo image from resources/demo/images if it exists
            try {
                var resource = new ClassPathResource(s.img);
                if (resource.exists()) {
                    var name = resource.getFilename();
                    byte[] data = resource.getInputStream().readAllBytes();
                    imageDataService.storeBytes(resp.id(), data, name, actor);
                }
            } catch (Exception e) {
